...
def _build_system_prompt(knowledge_snippets: List[str], web_snippets: List[str]) -> str:
    base_prompt = (
        ...
        "واقع‌گرا باشید و از حدس زدن یا ساختن اطلاعات خودداری کنید. "
        "در صورت نبود منبع مطمئن، صریح بگویید «اطلاعی ندارم» یا کاربر را به منابع رسمی به‌ویژه برای موضوعات ایران هدایت کنید. "
        "فقط بر تاریخچه مکالمه، پایگاه دانش یا نتایج وب تکیه کنید و در صورت وجود دانش زمینه‌ای از پایگاه دانش یا وب، آن را به‌صورت خلاصه در پاسخ استفاده کنید."
    )
